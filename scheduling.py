...
def fetch_processes(limit=5) -> pd.DataFrame:
    ...
    try:
        ...
        print("⚠️ Could not fetch live processes, using sample data instead.")
        sample = {
            "PID": [1, 2, 3, 4, 5],
...
def df_to_jobs(df: pd.DataFrame) -> List[Job]:
    ...
    for _, r in df.iterrows():
        burst = max(1, min(20, int(r.ETIMES)))  # cap burst time
        jobs.append(Job(
            pid=int(r.PID),
            name=r.COMMAND,
            burst=burst,
...
def priority_scheduling(jobs: List[Job]):
    ...
    while len(done) < len(js):
        ...
        if not ready:
            t += 1
            continue
        cur = min(ready, key=lambda x: x.priority)  # smaller = higher priority
        cur.start = t
        t += cur.burst
        cur.completion = t
        done.append(cur)
